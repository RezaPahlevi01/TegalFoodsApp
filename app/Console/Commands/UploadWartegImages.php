...
class UploadWartegImages extends Command
{
    protected $signature = 'warteg:upload-images';
    protected $description = 'Download images from Unsplash and upload to Cloudinary for warteg products';
    ...
    public function handle()
    {
        ...
        if (!$umkm) {
            $this->error('UMKM warteg tidak ditemukan.');
            return 1;
        }

        $this->info("UMKM: {$umkm->nama_umkm} (ID: {$umkm->id})");
        ...
        foreach ($products as $product) {
            if (!isset($this->images[$product->nama_makanan])) {
                $this->warn("  SKIP: {$product->nama_makanan} (no image URL)");
                continue;
            }

            $url = $this->images[$product->nama_makanan];
            $this->line("  Downloading: {$product->nama_makanan}...");

            $response = Http::timeout(30)->get($url);

            if (!$response->successful()) {
                $this->error("  FAIL download: {$product->nama_makanan} (HTTP {$response->status()})");
                $failed++;
                continue;
            }
            ...
            $tempFile = tempnam($tempDir, 'warteg_') . '.' . $ext;
            file_put_contents($tempFile, $response->body());

            $uploadedFile = new UploadedFile($tempFile, " warteg_{$product->id}.{$ext}", $contentType, null, true);

            $imagePath = $cloudinary->upload($uploadedFile, 'warteg');

            if ($imagePath) {
                $product->update(['gambar_url' => $imagePath]);
                $this->info("  OK: {$product->nama_makanan} -> " . substr($imagePath, 0, 60) . "...");
                $success++;
            } else {
                $this->error("  FAIL upload: {$product->nama_makanan}");
                $failed++;
            }
